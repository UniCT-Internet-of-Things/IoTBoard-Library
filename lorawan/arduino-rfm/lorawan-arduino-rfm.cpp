...
// define lora objet
LoRaWANClass _instance_lorawan;
...
bool LoRaWANClass::init(SPIClass& spi)
{
    _spi = &spi;
    // Lora Setting Class
    dev_class = CLASS_A;
...
bool LoRaWANClass::join(void)
{
    ...
    // join request
    LoRa_Send_JoinReq(&OTAA_Data, &LoRa_Settings, *_spi);
    // delay(900);
    // loop for <timeout> wait for join accept
    prev_millis = millis();
    do
    {
        join_status = LORA_join_Accept(&Buffer_Rx, &Session_Data, &OTAA_Data, &Message_Rx, &LoRa_Settings, *_spi);
...
void LoRaWANClass::sleep(void)
{
    ...
    RFM_Write(RFM_REG_OP_MODE, RFM_MODE_SLEEP, *_spi);
}

void LoRaWANClass::wakeUp(void)
{
    RFM_Switch_Mode(RFM_MODE_STANDBY, *_spi);
}
...
void LoRaWANClass::setDeviceClass(devclass_t dev_class)
{
    ...
    {
        RFM_Continuous_Receive(&LoRa_Settings, *_spi);
    }
...
{
    _instance_lorawan.setDeviceClass(CLASS_A); // start as class a device
...
void LoRaWANClass::setTxPower1(unsigned char power_idx)
{
    ...
    RFM_Write(RFM_REG_PA_CONFIG, RFM_Data, *_spi);
}
...
void LoRaWANClass::update(void)
{
    ...
    {
        //LoRaWAN TX/RX cycle
        LORA_Cycle(&Buffer_Tx, &Buffer_Rx, &RFM_Command_Status, &Session_Data, &OTAA_Data, &Message_Rx, &LoRa_Settings, &upMsg_Type, *_spi);
    ...
    if (LoRa_Settings.Mote_Class == CLASS_C)
    {
        ...
        {
            isRxDone = RFM_isRxDone(*_spi);
        }
        if (isRxDone)
        {
            LORA_Receive_Data(&Buffer_Rx, &Session_Data, &OTAA_Data, &Message_Rx, &LoRa_Settings, *_spi);
            if (Buffer_Rx.Counter != 0x00)
            {
